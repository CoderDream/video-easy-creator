 ...
 */
@Slf4j
public class OllamaTranslateUtil02 {
...
//    log.info("API 调用耗时: {}", ollamaResponse.getDuration());
  ...
  public static String translateText(String text) {
    ...
    //
    List<String> resutlSubtitleList = null;
    int retryTime = 0;
    while (CollectionUtil.isEmpty(resutlSubtitleList) || (
      resutlSubtitleList.size() != subtitleList.size() && retryTime < 10)) {
      if (CollectionUtil.isEmpty(resutlSubtitleList)) {
        resutlSubtitleList = new ArrayList<>();
      }
      if (retryTime > 0) {
        log.error(
          "返回数组大小: {} != 请求数组大小: {}"
            + CdConstants.TRANSLATE_PLATFORM_DEEP_SEEK + " 重试次数: {}",
          resutlSubtitleList.size(), subtitleList.size(), retryTime);
      }
      log.info("翻译前的文本: {}", message);
      String s = translateTextRaw(message);
      log.info("翻译后的文本: {}", s);
      String[] lines1 = s.split("\n"); // Split the string by newline character
      for (String subtitle : lines1) {
        if (StrUtil.isNotBlank(subtitle)) {
          resutlSubtitleList.add(subtitle);
        }
      }
      log.error(
        "返回数组大小: {} != 请求数组大小: {}",
        resutlSubtitleList.size(), subtitleList.size());
      retryTime++;
