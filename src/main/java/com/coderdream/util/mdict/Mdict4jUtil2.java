...
//            System.out.println(    Mdict4jUtil.getWordDetail(word, "oald"));;
      System.out.println(Mdict4jUtil2.genDictionaryEntry(word));
...
//
//      Optional<List<Entry<String, String>>> entries1 = Optional.ofNullable(
//          list)  // 获取字典条目
//        .filter(entries -> entries != null && !entries.isEmpty());
//
//      if (entries1.isPresent()) {
//        Optional<Entry<String, String>> stringStringEntry = Optional.ofNullable(
//            list)  // 获取字典条目
//          .map(entries -> entries.get(0));
//
//        if (stringStringEntry.isPresent()) {
//          Optional<String> s = stringStringEntry.map(Entry::getValue);
//
//          if (s.isPresent()) {
//            Optional<HtmlContentBean> htmlContentBean = s.map(
//              html -> parseHtmlByDictType(s.get(), dictType));
//          }
//        }
//      }
  ...
  public static DictionaryEntry genDictionaryEntry(String word) {
    HtmlContentBean htmlContentBean = Mdict4jUtil2.getHtmlContentBean(word,
      "oald");
