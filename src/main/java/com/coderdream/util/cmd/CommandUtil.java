...
public class CommandUtil {
   ...
   /**
     * 使用 FFmpeg 从 MP4 视频中提取 MP3 音频。
     * @param inputMp4    输入 MP4 文件的完整路径
     * @param outputMp3   输出 MP3 文件的完整路径
     */
    public static void extractAudioFromMp4(String inputMp4, String outputMp3) {
        List<String> command = new ArrayList<>();
        command.add("ffmpeg");
        command.add("-i");
        command.add(inputMp4);
        ...
        command.add("-acodec");
        command.add("libmp3lame"); // 设置音频编码器为 libmp3lame
        command.add("-ab"); // 设置音频比特率(可选)
        command.add("128k"); //例如设置为128kbps
        command.add(outputMp3);

        executeCommand(command);

        log.info("成功从 MP4 视频提取 MP3 音频: {}", outputMp3);
    }
...
//        String inputMp4 = "input.mp4"; // 替换为实际的 MP4 文件路径
//        String inputMp3 = "audio.mp3"; // 替换为实际的 MP3 文件路径
//        String outputMp4 = "output.mp4"; // 替换为期望的输出 MP4 文件路径
//
//        addAudioToMp4(inputMp4, inputMp3, outputMp4);

        String inputMp4 = "input.mp4"; // 替换为实际的 MP4 文件路径
        String outputMp3 = "output.mp3"; // 替换为期望的输出 MP3 文件路径

        CommandUtil.extractAudioFromMp4(inputMp4, outputMp3);
    }
