...
//    GetSixMinutesPpt.genPic(folderName); // 调用 genPic 方法生成图片，已注释掉
  ...
  public static void genPic(String folderName) {
    ...
    String outputDir =
      CommonUtil.getFullPath(folderName) + folderName + File.separator; // 定义输出目录
    ...
    PptToPng.savePptToPng(pptFilePath, outputDir, 1920, 1080); // 调用 PptToPng 工具类将 PPT 转换为 PNG 图片，并保存到输出目录，指定分辨率为 1920x1080
  }
  ...
  public static void process(String folderName) {
    LicenseUtil.loadLicense(MicrosoftConstants.PPTX_TO_OTHER); // 加载 Aspose.Slides 的 License，用于去除水印等限制
    ...
    try {
      ...
      for (WordInfoEntity wordInfoEntity : wordInfoEntityList) { // 遍历从 Excel 中读取的词汇信息
        ...
        BeanUtils.copyProperties(wordInfoEntity, wordInfo); // 使用 BeanUtils 将 WordInfoEntity 的属性复制到 WordInfo
        ++no; // 序号自增
      ...
      for (int j = 0; j < wordInfoList.size(); j += 10) { // 遍历词汇列表，每 10 个词汇分割成一组
        wordInfoListGroups.add(
          wordInfoList.subList(j, Math.min(j + 10, wordInfoList.size()))); // 将分割后的词汇信息添加到列表中
      }
      // 判断是否为空
      ...
      // 实例化Presentation类
      pres = new Presentation(presentationName); // 实例化 Presentation 类，加载 PPT 模板文件
      Map<String, String> props = new HashMap<>(); // 末日滚动：我们为什么喜欢末日滚动？
      props.put("folderName", folderName); // getArticleTitle // 将文件夹名称添加到属性映射中
      props.put("title",
        CdFileUtil.getArticleTitle(folderName)); // getArticleTitle // 从文件中读取文章标题，并添加到属性映射中
      List<VocInfo> vocInfoList = DictUtils.getVocInfoList(folderName); // 从文件中读取词汇信息
      int i = 1; // 定义序号
      ...
      for (VocInfo vocInfo : vocInfoList) { // 遍历词汇信息列表
        ...
        String mid = ""; // 定义分隔符
        log.info(
          sampleSentenceEn + " : " + sampleSentenceEn.length()); // 使用 log.info 记录英文例句和长度
        if (StrUtil.isNotEmpty(sampleSentenceEn)
          && sampleSentenceEn.length() < 50) { // 如果英文例句不为空，且长度小于 50
          mid = "\n"; // 则使用换行符作为分隔符
        }
        // TODO
        props.put("wordSen" + i, sampleSentenceEn + mid + sampleSentenceCn); // 将例句添加到属性映射中
        props.put("wordSenEn" + i, sampleSentenceEn); // 将英文例句添加到属性映射中
        props.put("wordSenCn" + i, sampleSentenceCn); // 将中文例句添加到属性映射中
        String midStr = " "; // 定义分隔符
        if (sampleSentenceEn.length() < 50 && sampleSentenceCn.length() < 50) { // 如果英文和中文例句长度都小于 50
          midStr = "\n"; // 则使用换行符作为分隔符
        }
      ...
      for (int j = 0; j < sizePpt; j++) { // 遍历每一张幻灯片
        log.info(
          "##############" + pres.getSlides().get_Item(j).getShapes().size()); // 使用 log.info 记录幻灯片中形状的数量
        ...
        for (IShape shp : shapesCopy) { // 遍历幻灯片中的每一个形状
          if (shp.getPlaceholder() != null) { // 判断形状是否是占位符
            ...
            switch (type) { // 根据占位符类型进行不同的处理
              case 1: // 文本占位符
                log.info(
                  "## \t " + shp.getPlaceholder().getType() + "这是占位符"); // 使用 log.info 记录占位符类型
                log.info(((IAutoShape) shp).getTextFrame().getText()); // 使用 log.info 记录占位符中的文本

                String text = ((IAutoShape) shp).getTextFrame().getText(); // 获取占位符中的文本
                // 去掉{{}}
                ...
                if (StrUtil.isNotEmpty(newText)) { // 如果替换文本不为空
                  ((IAutoShape) shp).getTextFrame().setText(newText); // 将占位符中的文本替换为新的文本
                }

                break;
              case 15: // 图片占位符
                log.info(
                  "## \t " + shp.getPlaceholder().getType() + "这是图片"); // 使用 log.info 记录占位符类型
                ...
                if (bufferedImage != null) { // 如果图片读取成功
                  try {
                    ...
                    ImageIO.write(bufferedImage, "png", os); // 将 BufferedImage 写入 ByteArrayOutputStream
                    try (InputStream is = new ByteArrayInputStream(
                      os.toByteArray())) { // 使用 try-with-resources 创建 InputStream
                      // 使用推荐方法 addImage(InputStream)
                      IPPImage pptxImage = pres.getImages().addImage(is); // 将 InputStream 中的图片添加到 PPT 中
                      ...
                      shape.getEffectFormat().enableSoftEdgeEffect(); // 启用柔化边缘效果
                      shape.getEffectFormat().getSoftEdgeEffect().setRadius(15); // 设置柔化边缘的半径
                      ...
                      // 设置图片填充
                      shape.getFillFormat().setFillType(FillType.Picture); // 设置填充类型为图片
                      IPictureFillFormat pictureFillFormat = shape.getFillFormat()
                        .getPictureFillFormat(); // 获取图片填充格式
                      pictureFillFormat.getPicture().setImage(pptxImage); // 设置填充图片
...
//            break;
          default: // 其他情况，可以添加异常处理或者默认行为
            log.warn("WordInfoListGroups size is {}, which is not supported.", groupSize);
            break;
    ...
    for (IShape shp : sld.getShapes()) { // 遍历幻灯片中的每一个形状
      ...
      if (shp instanceof ITable) { // 判断形状是否是表格
        ...
        for (WordInfo wordInfo2 : wordInfoList1) { // 遍历词汇信息列表
          y++; // 行号自增
          tbl.get_Item(0, y).getTextFrame().setText(wordInfo2.getNo() + ""); // 设置序号
          tbl.get_Item(1, y).getTextFrame()
            .setText(wordInfo2.getWord()); // 设置单词

          tbl.get_Item(2, y).getTextFrame().setText(wordInfo2.getUk()); // 设置音标
          tbl.get_Item(3, y).getTextFrame().setText(wordInfo2.getComment()); // 设置注释
