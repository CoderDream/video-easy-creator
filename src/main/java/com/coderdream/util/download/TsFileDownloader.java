...
public class TsFileDownloader {
    ...
    private static final int RETRY_DELAY_MS = 1000; // 重试间隔时间 (毫秒)
        ...
        try {
            ...
            for (int i = startIndex; i <= endIndex; i++) {
                ...
                if (Files.exists(tempFile)) {
                    ...
                    try {
                        downloadTsFileWithRetry(tsFileUrl, tempFile, MAX_RETRIES, RETRY_DELAY_MS); // 下载 TS 文件到指定路径
                        downloadedFiles.add(tempFile);
                        log.info("文件 {} 下载成功", tsFileUrl);
                    } catch (IOException e) {
                        log.error("下载文件 {} 失败 (多次重试后): {}", tsFileUrl, e.getMessage(), e);
     ...
     * @throws IOException 下载过程中发生异常，如果所有重试都失败
     */
    private static void downloadTsFileWithRetry(String tsFileUrl, Path tempFile, int maxRetries, int retryDelayMs) throws IOException {
        int retryCount = 0;
        while (retryCount < maxRetries) {
            try {
                downloadTsFile(tsFileUrl, tempFile); // 尝试下载
                return; // 下载成功，直接返回
            } catch (IOException e) {
                retryCount++;
                log.warn("下载文件 {} 失败 (第 {} 次重试): {}", tsFileUrl, retryCount, e.getMessage());
                if (retryCount < maxRetries) {
                    try {
                        Thread.sleep(retryDelayMs); // 休眠一段时间后重试
                    } catch (InterruptedException ie) {
                        Thread.currentThread().interrupt(); // 恢复中断状态
                        throw new IOException("下载被中断", ie); // 抛出异常
                    }
                } else {
     ...
     *
     * @param tsFilePath  TS 文件路径
     * @param mp4FilePath MP4 文件路径
     * @return 转换结果，"success" 表示成功，其他表示失败原因
     */
    public static String convertTsToMp4(String tsFilePath, String mp4FilePath) {
        List<String> command = new ArrayList<>();
        command.add("ffmpeg");
        ...
        try {
            ...
            if (exitCode == 0) {
                log.info("TS 文件转换为 MP4 文件成功，输出文件：{}", mp4FilePath);
                return "success";
            } else {
                log.error("FFmpeg 进程执行失败，退出代码: {}", exitCode);
                // 尝试读取错误流
                ...
                return "FFmpeg 转换失败，退出代码: " + exitCode;
            }
        } catch (IOException | InterruptedException e) {
            log.error("执行 FFmpeg 命令时发生异常: ", e);
            return "执行 FFmpeg 命令时发生异常: " + e.getMessage();
        } finally {
            if (process != null) {
                process.destroy();
            }
    ...
    public static void main(String[] args) {
        ...
        String elapsedTime = TsFileDownloader.downloadAndMergeTsFilesWithFFmpeg(baseUrl, startIndex, endIndex, tempDir, outputFile, suffix);
