...
@Slf4j
public class HighResImageVideoUtil7 {

  private static final int LINE_SPACING = 40;   // 垂直间距，设置为原来的两倍
...
//    fullPath = fileName + ".txt";
  ...
  public static List<File> generateImages(List<SentenceVO> sentenceVOs,
    ...
    for (int i = 0; i < Objects.requireNonNull(sentenceVOs).size(); i++) {
      ...
      currentY = drawWrappedText(g2d, englishFont, Color.WHITE,
          sentenceVO.getEnglish(), currentY, width);

      currentY = drawWrappedText(g2d, phoneticsFont, Color.YELLOW,
          sentenceVO.getPhonetics(), currentY + LINE_SPACING, width);

      drawWrappedText(g2d, chineseFont, Color.WHITE,
          sentenceVO.getChinese(), currentY + LINE_SPACING, width);
  ...
  private static int drawWrappedText(Graphics2D g2d, Font font, Color color,
      String text, int y, int maxWidth) {
    g2d.setFont(font);
    g2d.setColor(color);
  ...
  public static void main(String[] args) throws Exception {
    ...
    HighResImageVideoUtil7.generateImages(sentenceVOs, imagePath, fileName,
        outputDir);
  }
}
