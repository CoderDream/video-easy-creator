...
@Slf4j
public class HighResImageGenUtil2a {
  ...
  static class ColumnConfig {
    ...
    public ColumnConfig(int width, int height, Font font, Color color) {
      this.width = width;
      this.height = height;
      this.font = font;
      this.color = color;
  ...
  public static List<File> generateImagesFromWordInfoList(
    ...
    ColumnConfig[] columnConfigs = {
      new ColumnConfig(100, 40, new Font("微软雅黑", Font.BOLD, 20),
        Color.BLACK), // 单词
      new ColumnConfig(120, 40, new Font("Arial", Font.PLAIN, 18),
        Color.DARK_GRAY), // 英音
      new ColumnConfig(250, 60, new Font("Times New Roman", Font.PLAIN, 16),
        Color.BLACK), // 释义 (允许高度更大)
      new ColumnConfig(80, 40, new Font("Consolas", Font.PLAIN, 16),
        Color.GREEN),  // 等级
      new ColumnConfig(60, 40, new Font("Monospace", Font.BOLD, 16), Color.RED)
      // 次数
    };
    ...
    for (WordInfo wordInfo : wordInfoList) {
      BufferedImage bufferedImage = new BufferedImage(templateWidth,
        templateHeight, BufferedImage.TYPE_INT_RGB);
      ...
      // 绘制表头 (仅在第一页或新页绘制)
      if (currentRowY == tableY) {
        drawHeader(g2d, columnConfigs, tableX, tableY);
      }
      ...
      for (int j = 0; j < data.length; j++) {
        drawWrappedText(g2d, columnConfigs[j].font, columnConfigs[j].color,
          ...
          columnConfigs[j].height);
        currentX += columnConfigs[j].width + 10; // 加上列间距
      }
  ...
  private static void drawHeader(Graphics2D g2d, ColumnConfig[] columnConfigs,
    ...
    for (int i = 0; i < headers.length; i++) {
      g2d.setFont(columnConfigs[i].font);
      ...
      currentX += columnConfigs[i].width + 10; // 加上列间距
    }
  }
...
//        1)
    ...
    // 调用生成方法
    HighResImageGenUtil2a.generateImagesFromWordInfoList(generateWordInfoList(),
      imagePath,
      fileName,
      outputDir);
  }
