...
@Slf4j
public class HighResImageGenUtil2 {
    ...
    private static final int COLUMN_WIDTH_CN = 920;
    ...
    private static final int BOTTOM_MARGIN = 135; // 底部预留空间
    private static final int HEADER_DATA_GAP = 20; // 表头和数据之间的间距

    private static final  int FONT_SIZE = 32;
    // 列配置 (可自定义)
    private static final ColumnConfig[] columnConfigs = {
            new ColumnConfig(COLUMN_WIDTH_WORD, new Font("微软雅黑", Font.BOLD, FONT_SIZE),
                    Color.BLACK), // 单词
            new ColumnConfig(COLUMN_WIDTH_UK, new Font("Arial Unicode MS", Font.PLAIN, FONT_SIZE),
                    Color.BLACK), // 英音
            new ColumnConfig(COLUMN_WIDTH_CN,
                    new Font("微软雅黑", Font.PLAIN, FONT_SIZE), Color.BLACK), // 释义
            new ColumnConfig(COLUMN_WIDTH_LEVEL, new Font("微软雅黑", Font.PLAIN, FONT_SIZE),
                    Color.BLACK),  // 等级
            new ColumnConfig(COLUMN_WIDTH_TIMES, new Font("微软雅黑", Font.BOLD, FONT_SIZE),
                    Color.BLACK)    // 次数
    };
    ...
    public static List<File> generateImagesFromWordInfoList(
        ...
        Font headerFont = new Font("微软雅黑", Font.BOLD, 32); // 统一表头字体
        int headerHeight = 0;
        for (int i = 0; i < wordInfoList.size(); i++) {
            ...
            if (i == 0 || currentRowY + rowHeight + 10 > templateHeight - BOTTOM_MARGIN) {
                ...
                if (bufferedImage != null) {
                    // 保存上一页图片
                    g2d.dispose();
                    int numberOfPages = currentPage + 9;
                    File outputFile = new File(
                            outputDir + File.separator + fileName + MessageFormat.format(
                                    "{0,number,00}", numberOfPages) + ".png");
                    try {
                ...
                // 绘制表头 (每页都绘制)
                headerHeight = drawHeader(g2d, columnConfigs, tableX, pageStartRowY, headerFont);
                currentRowY = tableY + headerHeight + HEADER_DATA_GAP;  // 重置当前行Y坐标，并将数据向下偏移表头高度
            ...
            for (int j = 0; j < data.length; j++) {
                drawWrappedText(g2d, columnConfigs[j].font, columnConfigs[j].color,
                        ...
                        rowHeight, j == 3 || j == 4); //  传递是否需要居中
                currentX += columnConfigs[j].width + COLUMN_GAP; // 加上列间距
            }


            // 分页逻辑
            currentRowY += rowHeight + 15; //加上行间距
    ...
    private static int drawHeader(Graphics2D g2d, ColumnConfig[] columnConfigs,
                                 int x, int y, Font headerFont) {
        String[] headers = {"单词", "英音", "释义", "等级", "出现次数"};
        int currentX = x;
        int headerHeight = 0;
        for (int i = 0; i < headers.length; i++) {
            ...
            float headerX = currentX + (columnConfigs[i].width - textWidth) / 2; // 计算居中 x 坐标

            g2d.drawString(headers[i], headerX, y + (int) bounds.getHeight());
            headerHeight = Math.max(headerHeight, (int) bounds.getHeight());
    ...
    private static void drawWrappedText(Graphics2D g2d, Font font, Color color,
                                       String text, int x, int y, int maxWidth, int maxHeight, boolean center) {
        g2d.setFont(font);
        g2d.setColor(color);
        ...
        while (lbm.getPosition() < aci.getEndIndex() && currentY < y + maxHeight) {
            ...
            float textWidth = (float) tl.getBounds().getWidth();
            float currentX = x; // 初始化 x
           if (center) {
             currentX = x + (maxWidth - textWidth) / 2; // 如果需要居中，计算 x 的偏移量
           }
...
//    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<WordInfoEntity> wordInfoEntityList = CdExcelUtil.getAdvancedWordList(folderName);// generateWordInfoList();
        List<WordInfo> wordInfoList = new ArrayList<>();
        WordInfo wordInfo;
        ...
        // 调用生成方法
        HighResImageGenUtil2.generateImagesFromWordInfoList(wordInfoList,
                imagePath,
                fileName,
                outputDir);
    }
