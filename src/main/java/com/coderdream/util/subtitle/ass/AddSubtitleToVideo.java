 ...
 */
@Slf4j
public class AddSubtitleToVideo {
     ...
     * @throws IOException          如果执行ffmpeg命令失败，或者文件不存在
    ...
    public static void addSubtitle(String videoPath, String subtitlePath) throws IOException, InterruptedException {
        ...
        if (!subtitleFile.exists() || !subtitleFile.isFile()) {
            log.error("字幕文件不存在: {}", subtitlePath);
            throw new IOException("字幕文件不存在: " + subtitlePath);
        }
        ...
        command.add("ass=" + Paths.get(subtitlePath).toAbsolutePath().toString());    // 添加字幕，使用绝对路径
