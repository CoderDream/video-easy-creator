...
public class SubtitleParser1 {
    ...
    private static List<SubtitleEntity> parseText(String filePath) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            for (String sentence : sentences) {
               ...
               //对句子进行逗号切割
                List<String> subSentences = splitSentenceByComma(sentence.trim());
    ...
    /**
     * 如果句子长度超过20个字符，则以逗号分割句子
     *
     * @param sentence 输入句子
     * @return 分割后的句子列表
     */
    private static List<String> splitSentenceByComma(String sentence){
         List<String> subSentences = new ArrayList<>();

        if(sentence.length() > 20){
           String[] split = sentence.split("(?<=,)"); // 使用正则表达式，保留逗号
            for(String sub : split){
                if (!sub.trim().isEmpty()){
                    subSentences.add(sub.trim());
                }
            }
            return subSentences;
