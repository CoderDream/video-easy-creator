  ...
  private static List<SubtitleEntity> parseText(String filePath) {
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        //第二部分：全书核心内容——
        //结束语
        if(line.startsWith("第一部分：作者简介——") ){
          line = line.replace("第一部分：作者简介——", "").trim();
        }
        if( line.startsWith("第二部分：全书核心内容——") ){
          line = line.replace("第二部分：全书核心内容——", "").trim();
        }
        if( line.startsWith("第三部分：总结——") ){
          line = line.replace("第三部分：总结——", "").trim();
        }
        if(line.startsWith("结束语")){
          continue;
        }
...
//      sentences = Pattern.compile("(?<=[。！？；：])|(?=“)").split(text);
  ...
  private static String replaceNumberFormat(String text) {
    String[] chineseNumerals = {"一、", "二、", "三、", "四、", "五、", "六、", "七、",
      "八、", "九、", "十、"};
   ...
   */
  private static void saveSubtitlesToFile(List<SubtitleEntity> subtitles,
