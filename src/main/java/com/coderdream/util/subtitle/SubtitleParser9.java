...
public class SubtitleParser9 {
            ...
            for (String sentence : sentences) {
                ...
                List<String> subSentences = splitSentence(sentence.trim());
                 for(String subSentence : subSentences){
                    ...
                    subtitle.setSubtitle(subSentence.trim().replaceAll("\\s+", ""));
    ...
    private static List<String> splitSentence(String sentence) {
        List<String> result = new ArrayList<>();
         if (sentence.length() > 20) {
             List<String> subSentences = Arrays.stream(Pattern.compile("(?<=[，])").split(sentence))
                     .map(String::trim)
                     .filter(sub -> !sub.isEmpty())
                     .collect(Collectors.toList());
            for (String subSentence : subSentences) {
                result.addAll(splitBySpace(subSentence));
            }
            return result;
        } else {
            return  splitBySpace(sentence);
        }
    }
     ...
     * @return 分割后的句子列表
     */
    private static List<String> splitBySpace(String sentence) {
