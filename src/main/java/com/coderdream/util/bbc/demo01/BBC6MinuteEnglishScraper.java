package com.coderdream.util.bbc.demo01;
...
public class BBC6MinuteEnglishScraper {
    ...
    private static String extractScript(Document doc) {
        ...
        Element introductionSection = doc.selectFirst(".widget-richtext > div.text > p:first-child");
        if (introductionSection != null) {
            script.append(introductionSection.text()).append("\n\n");
        }
        Elements scriptSection = doc.select(".widget-richtext > div.text > p");
        if (scriptSection != null) {
            // 假设脚本位于 <p> 标签中
            for (Element paragraph : scriptSection) {
                script.append(paragraph.text()).append("\n\n");
            }
        }
