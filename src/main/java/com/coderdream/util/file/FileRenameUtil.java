...
public class FileRenameUtil {
  ...
  /**
   * 重命名文件。 如果新文件路径与旧文件路径的目录不同，则执行移动并重命名。 默认不覆盖已存在的目标文件。
   *
  ...
  public static boolean renameFile(String oldFilePath, String newFilePath) {
    ...
    try {
      ...
      if (!oldFile.exists()) {
        ...
        // 这种情况理论上不应该发生，但以防万一
        log.warn("文件可能已复制到 '{}'，但旧文件 '{}' 仍然存在。", newFilePath,
          oldFilePath);
        return false; // 或者根据业务逻辑判断是否算成功
      }
    } catch (IORuntimeException e) {
      // IORuntimeException 是 HuTool 对 IOException 的封装
      log.error("文件重命名/移动操作时发生 IO 异常: '{}' -> '{}'", oldFilePath,
        newFilePath, e);
      return false;
    } catch (Exception e) {
      // 捕获其他可能的运行时异常
      log.error("文件重命名/移动操作时发生未知异常: '{}' -> '{}'", oldFilePath,
        newFilePath, e);
      return false;
    }
  }
   ...
   * @return 如果重命名（或移动并重命名）成功，返回 true；否则返回 false。
   */
  public static boolean renameFileOverride(String oldFilePath,
    String newFilePath) {
    ...
    try {
      ...
      if (!oldFile.exists()) {
        log.info("文件重命名/移动并覆盖成功: '{}' -> '{}'", oldFilePath,
          newFilePath);
        return true;
      } else {
        log.warn("文件已覆盖到 '{}'，但旧文件 '{}' 仍然存在。", newFilePath,
          oldFilePath);
        return false;
      }
    } catch (IORuntimeException e) {
      log.error("文件重命名/移动(覆盖)操作时发生 IO 异常: '{}' -> '{}'",
        oldFilePath, newFilePath, e);
      return false;
    } catch (Exception e) {
      log.error("文件重命名/移动(覆盖)操作时发生未知异常: '{}' -> '{}'",
        oldFilePath, newFilePath, e);
      return false;
    }
  }
  ...
  public static void main(String[] args) {
    ...
    String newMp4FileName =
      testDir + File.separator + "20250408_raw222.png"; // 移动到子目录并重命名
//    String existingFileName = testDir + File.separator
//      + "20250408_raw.png"; // testDir + "existing_file.txt";
//
//    // 准备测试环境
...
//    success = renameFile(testDir + "non_existent_file.avi",
//      testDir + "some_new_name.avi");
...
//    success = renameFile(tempOldFile,
//      existingFileName); // 尝试重命名为已存在的 existing_file.txt
//    log.info("测试场景3结果: {}", success ? "成功" : "失败");
//    log.info("检查新文件（目标）是否存在: {}", FileUtil.exist(existingFileName));
//    log.info("检查临时旧文件是否还存在: {}",
//      FileUtil.exist(tempOldFile)); // 应该还存在

    // 测试场景4：新文件已存在（允许覆盖）
    log.info("--- 测试场景4：新文件已存在（允许覆盖） ---");
    // 使用上一步的 tempOldFile 和 existingFileName
    boolean success = renameFileOverride(oldMp4FileName, newMp4FileName); // 允许覆盖
//    log.info("测试场景4结果: {}", success ? "成功" : "失败");
//    log.info("检查新文件（目标）是否存在: {}",
//      FileUtil.exist(existingFileName)); // 应该存在（内容已被覆盖）
//    log.info("检查临时旧文件是否还存在: {}",
//      FileUtil.exist(tempOldFile)); // 应该不存在了
//
//    // 测试场景5：路径为空
//    log.info("--- 测试场景5：路径为空 ---");
