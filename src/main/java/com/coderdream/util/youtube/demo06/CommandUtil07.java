...
public class CommandUtil07 {
  ...
  public static boolean downloadMedia(String videoLink, String formatId,
    ...
    // 添加代理参数
    if (useProxy) {
      String proxyString = YouTubeApiUtil.getProxyArgumentString(); // 假设此方法返回 "--proxy http://127.0.0.1:1080" 或类似格式 "--proxy http://127.0.0.1:1080";//
...
//     }
  ...
  private static String findBestFormat(List<String> formats,
    ...
    for (String format : formats) {
      if (isVideo) {
        ...
        if (format.contains(targetResolution) && format.contains(
          "video only")) {
    ...
    if (bestFormatInfo == null) {
      log.warn("在 {} 个格式中未能找到 {} 格式 (分辨率: {})", formats.size(),
        isVideo ? "视频" : "音频", targetResolution);
    }
  ...
  private static Charset getConsoleCharset() {
        ...
        if (!finished) {
          ...
          if (matcher.find()) {
            ...
            try {
              ...
              log.warn("不支持的编码名称 '{}' (来自 chcp)，将使用默认编码。",
                charsetName);
            } catch (java.nio.charset.IllegalCharsetNameException e) {
              log.warn("非法的编码名称 '{}' (来自 chcp)，将使用默认编码。",
                charsetName);
            }
          } else {
    ...
    log.warn("无法准确获取控制台编码，将使用系统默认编码: {}",
      defaultCharset.name());
  ...
  private static class StreamGobbler implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        log.trace("StreamGobbler 读取异常 (可能是进程结束): {}",
          e.getMessage());
      } finally {
        try {
          reader.close();
