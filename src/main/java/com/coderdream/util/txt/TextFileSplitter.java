...
public class TextFileSplitter {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (chapterMatcher.find()) {
          if (writer != null) {
            try {
              writer.close();
              log.debug("当前章节文件写入完成，文件名为:{}",
                currentChapterDirName);
            } catch (IOException e) {
          ...
          chapterCount++;
          currentChapterDirName = String.format("Chapter%03d",
            chapterCount);  // 创建形如 Chapter001 的文件夹名
          Path chapterDir = parentDir.resolve(currentChapterDirName);
          if (!Files.exists(chapterDir)) {
            Files.createDirectory(chapterDir);
            log.debug("创建输出文件夹,文件夹名称为：{}", chapterDir);
          }
          String outputFileName = String.format("Chapter%03d_temp.txt",
            chapterCount); // 输出文件名字如 Chapter001.txt
          File outputFile = new File(chapterDir.toString(), outputFileName);
          outputFiles.add(outputFile.getAbsolutePath());
          writer = new BufferedWriter(new FileWriter(outputFile));
          log.debug("创建新章节输出文件,文件名为:{}",
  ...
  public static void main(String[] args) {
    ...
    List<String> outputFiles = TextFileSplitter.splitFileByChapter(
      inputFilePath);
