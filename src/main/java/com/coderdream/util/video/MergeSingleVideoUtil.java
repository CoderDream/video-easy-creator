...
@Slf4j
public class MergeSingleVideoUtil {
    ...
    try {
      ...
      try (BufferedWriter writer = new BufferedWriter(
        ...
        for (int i = 0; i < size; i++) {
          ...
          log.info("合并视频: {}，中文路径: {}, 英文路径: {}", indexStr,
            videoCnFile.getAbsolutePath(), videoEnFile.getAbsolutePath());
          mergeVideos(videoCnFile, videoEnFile, fileName, indexStr);
        }
      }
   ...
   * 合并所有生成的视频文件
   *
   * @param videoCnFile 中文视频文件
   * @param videoEnFile 英文视频文件
   */
  public static File mergeVideos(File videoCnFile,
    File videoEnFile, String fileName, String indexStr) {
    String mergeFilename =
    ...
    try {
      ...
      // 检查视频帧率，确保一致
      double frameRate = getFrameRate(videoCnFile); // 假设所有视频帧率一致
      log.info("原始视频帧率为: {}", frameRate);
      ...
      try (BufferedReader reader = new BufferedReader(
        ...
        log.error("读取FFmpeg输出时出错{}", e.getMessage(), e);
      }
