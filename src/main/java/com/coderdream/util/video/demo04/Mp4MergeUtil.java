...
public class Mp4MergeUtil {
     ...
     */
    public static void mergeMp4Files(String folderPath, String chapterName, Path inputDir, Path outputDir) {
        // 记录方法调用的开始时间
        ...
        try {
            ...
            List<Path> mp4Files = Files.walk(inputDir)
                    .filter(file -> file.toString().endsWith(".mp4")).sorted(Comparator.comparing(Path::getFileName)).collect(Collectors.toList());
            ...
            // 创建存放list.txt文件的目录
            String listFilePath = folderPath + File.separator + chapterName + File.separator + "video_list_file";
...
//      "D:/0000/EnBook002/Chapter015/video_merge_" + count);
////    Path outputDir = Paths.get(
////      "D:/0000/EnBook002/Chapter015/video_merge" + count + 1);
//    // 获取文件夹中的所有文件（不包括子文件夹中的文件）
//    List<File> files = FileUtil.loopFiles(inputDir,
//      file -> !file.isDirectory());
//    while (files.size() > 1) {
...
//        "D:/0000/EnBook002/Chapter015/video_merge_" + count);
//      Path outputDir = Paths.get(
//        "D:/0000/EnBook002/Chapter015/video_merge_" + (count + 1));
//
//      // 确保输出目录存在
