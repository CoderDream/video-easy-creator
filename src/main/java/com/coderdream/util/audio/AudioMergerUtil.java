...
public class AudioMergerUtil {
  ...
  private static String createListFile(List<String> wavFiles, String inputDir,
    ...
    String pageFile = "D:\\0000\\bgmusic\\page.wav"; // 背景音乐文件路径，这里应为绝对路径，或写在配置中
    try (BufferedWriter writer = new BufferedWriter(
      new FileWriter(listFilePath))) {

      if (addBeginMusic) {
        writer.write("file '" + "D:\\0000\\bgmusic\\eng.wav" + "'"); // 写入背景音乐
        writer.newLine();
        writer.write("file '" + "D:\\0000\\bgmusic\\chn.wav" + "'"); // 写入背景音乐
        writer.newLine();
      }
