...
@Slf4j
public class AudioMergerMixBatchV20230317 {
   ...
   */
  public static File mergeWavFiles(String inputDirCn, String inputDirEn,String inputDirEnSlowly,
    ...
    List<String> wavFilesEnSlowly = listWavFiles(inputDirEnSlowly); // 获取英文音频文件列表
    if (wavFilesEn.isEmpty()) {
      log.warn("英文目录{}下没有找到wav文件", inputDirEnSlowly);
      return null;
    }
    ...
    List<String> listFiles = createListFile(wavFilesEn, wavFilesCn, wavFilesEnSlowly,
  ...
  private static List<String> createListFile(List<String> wavFilesEn,
    List<String> wavFilesCn, List<String> wavFilesEnSlowly, String outputFilePath) {
    ...
    String wavFileNameEnSlowly;
    for (int i = 0; i < size; i++) {
      wavFileNameCn = wavFilesCn.get(i);
      wavFileNameEn = wavFilesEn.get(i);
      wavFileNameEnSlowly = wavFilesEnSlowly.get(i);
      ...
      try (BufferedWriter writer = new BufferedWriter(
        ...
        writer.newLine();
        writer.write("file '" + wavFileNameEnSlowly + "'");
        writer.newLine();
        writer.write("file '" + wavFileNameEn + "'");
        writer.newLine();
      } catch (IOException e) {
