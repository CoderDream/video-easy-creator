...
@Slf4j
public class AudioFolderDurationUtil2 {
  ...
  public static File createAudioDurationFileList(String folderName) {
    ...
    // 创建合并文件列表
    String durationFileName = folderName + "duration.txt";
    ...
    try (BufferedWriter writer = new BufferedWriter(
      ...
      for (int i = 0; i < size; i++) {
        File audioFile = new File(
          folderName + File.separator + totalFileNames.get(i));
        // 提交任务，计算音频时长并保存到未来任务中
        futures.add(executorService.submit(() -> {
          return audioFile.getName() + "\t" + getAudioDurationWithRetry(
            audioFile) + " \n";
        }));
      }
