...
public class JsonToDialogDualSentenceEntityConverter {
    ...
    public static void main(String[] args) {
         String jsonString =
           ...
           + "		]\n"
            + "	}\n"
            + "]";
        List<DialogDualSentenceEntity> dialogDualSentenceEntities =
            convertJsonToDialogDualSentenceEntities(jsonString);

        if (dialogDualSentenceEntities != null) {
            dialogDualSentenceEntities.forEach(item -> {
            ...
            System.out.println("Failed to convert JSON to List<DialogDualSentenceEntity>.");
        }
    }


    public static List<DialogDualSentenceEntity> convertJsonToDialogDualSentenceEntities(String jsonString) {
        if (jsonString == null || jsonString.trim().isEmpty()) {
            return null;
        }
        JSONArray jsonArray = JSONUtil.parseArray(jsonString);
        List<DialogDualSentenceEntity> dialogDualSentenceEntities = new ArrayList<>();
        DialogDualSentenceEntity currentEntity = new DialogDualSentenceEntity();

        for (int i = 0; i < jsonArray.size(); i++) {
            JSONObject jsonObject = jsonArray.getJSONObject(i);
            String speaker = jsonObject.getStr("speaker");
            List<String> sentences = jsonObject.getBeanList("sentences", String.class);


              if(speaker.equalsIgnoreCase("A") || speaker.equalsIgnoreCase("Mary") || speaker.equalsIgnoreCase("Ann")){
                   if (currentEntity.getHostAEn() == null ) {
                     ...
                     if(currentEntity.getContentAEn() == null){
                       currentEntity.setContentAEn(new ArrayList<>(sentences));
                    }else {
                          currentEntity.getContentAEn().addAll(sentences);
                      }
              }else if (speaker.equalsIgnoreCase("B") || speaker.equalsIgnoreCase("Tom") || speaker.equalsIgnoreCase("Shirley") || speaker.equalsIgnoreCase("Rose")|| speaker.equalsIgnoreCase("David") || speaker.equalsIgnoreCase("雪莉") || speaker.equalsIgnoreCase("汤姆") || speaker.equalsIgnoreCase("罗丝") || speaker.equalsIgnoreCase("大卫")  || speaker.equalsIgnoreCase("安")) {
                  if (currentEntity.getHostBEn() == null ) {
                    ...
                    if(currentEntity.getContentBEn() == null){
                         currentEntity.setContentBEn(new ArrayList<>(sentences));
                    }else{
                         currentEntity.getContentBEn().addAll(sentences);
                    }
                 }

            if(i == jsonArray.size() -1 ||(i + 1 < jsonArray.size()  &&  !jsonArray.getJSONObject(i + 1).getStr("speaker").equalsIgnoreCase(speaker)) ){
                dialogDualSentenceEntities.add(currentEntity);
                currentEntity = new DialogDualSentenceEntity();
            }

        }
        return dialogDualSentenceEntities;
    }
}
