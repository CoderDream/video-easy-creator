...
public class SrtParser {
    ...
    public static List<String> parseSrtFile(String filePath) throws IOException {
        ...
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(filePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.isEmpty() || line.matches("\\d+") || line.matches("\\d{2}:\\d{2}:\\d{2},\\d{3} --> \\d{2}:\\d{2}:\\d{2},\\d{3}")) {
                    continue;
                }
                ...
                if (splitSentences.size() > 1) {
                    for (int i = 0; i < splitSentences.size() - 1; i++) {
                        sentences.addAll(splitLongSentence(removeSpeakerPrefix(splitSentences.get(i))));
                    }
            ...
            if (currentSentence.length() > 0) {
                sentences.addAll(splitLongSentence(removeSpeakerPrefix(currentSentence.toString().trim())));
            }
        } catch (IOException e) {
    ...
    private static List<String> splitLongSentence(String sentence) {
        ...
        for (int i = 0; middle - i >= 0 && middle + i < sentence.length(); i++) {
            if (commaIndex == -1 && middle - i >= 0 && sentence.charAt(middle - i) == ',') {
                commaIndex = middle - i;
            }
            if (commaIndex == -1 && middle + i < sentence.length() && sentence.charAt(middle + i) == ',') {
                commaIndex = middle + i;
            }
            if (commaIndex != -1) {
                break; // 找到逗号就停止
        ...
        if (commaIndex == -1) {
            for (int i = 0; middle - i >= 0 && middle + i < sentence.length(); i++) {
                if (semicolonIndex == -1 && middle - i >= 0 && sentence.charAt(middle - i) == ';') {
                    semicolonIndex = middle - i;
                }
                if (semicolonIndex == -1 && middle + i < sentence.length() && sentence.charAt(middle + i) == ';') {
                    semicolonIndex = middle + i;
                }
                if (semicolonIndex != -1) {
                    break;//找到分号就停止
        ...
        if (splitIndex != -1) {
            ...
            if (!part1.isEmpty() && !part1.replaceAll("\\s+", "").equals(sentence.replaceAll("\\s+", ""))) {
                result.addAll(splitLongSentence(part1));
            }
            if (!part2.isEmpty() && !part2.replaceAll("\\s+", "").equals(sentence.replaceAll("\\s+", ""))) {
            ...
            if (sentence.length() > MAX_SENTENCE_LENGTH) {
                log.warn("Sentence exceeds max length and cannot be split: {}", sentence);
            }
            result.add(sentence);
        }
