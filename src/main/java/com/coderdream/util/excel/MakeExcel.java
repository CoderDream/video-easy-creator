...
//        MakeExcel.listFill(templateFileName, excelFileName, sheetName, wordInfoList);
   ...
   * @param wordInfoList
   */
  public static void fillWordEntityList(String templateFileName,
    String fileName, String sheetName, List<WordInfo> wordInfoList) {
...
//        contentWriteCellStyle.setWriteFont(contentWriteFont);
  ...
  public static void listFill(String templateFileName, String fileName,
    String sheetName1,
    List<WordInfo> wordInfoList1, String sheetName2,
    List<WordInfo> wordInfoList2, String sheetName3,
    List<WordInfo> wordInfoList3, String sheetName4,
    List<WordInfo> wordInfoList4) {
    // 模板注意 用{} 来表示你要用的变量 如果本来就有"{","}" 特殊字符 用"\{","\}"代替
    // 填充list 的时候还要注意 模板中{.} 多了个点 表示list
    // 如果填充list的对象是map,必须包涵所有list的key,哪怕数据为null，必须使用map.put(key,null)
...
//            .sheet(sheetName2).doFill(wordInfoList2);
  ...
  public static void fillReportWithEasyExcel(HttpServletResponse response,
    Map<String, List<?>> sheetAndDataMap,
    ...
    try {
      ...
      for (Map.Entry<String, List<?>> entry : sheetAndDataMap.entrySet()) {
        ...
        WriteSheet writeSheet = EasyExcel.writerSheet(
          Integer.valueOf(entry.getKey())).build();
