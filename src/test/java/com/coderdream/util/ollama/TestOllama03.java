...
public class TestOllama03 {
    ...
    static {
        ...
        try (InputStream input = TestOllama03.class.getClassLoader().getResourceAsStream("application.properties")) {
            if (input == null) {
            ...
            OPENAI_TEMPERATURE = Double.parseDouble(prop.getProperty("spring.ai.openai.chat.options.temperature"));


        } catch (IOException ex) {
            ex.printStackTrace();
            throw new RuntimeException("Error loading application.properties", ex); // Crucial!
        } catch (NumberFormatException ex) {
            System.err.println("Error parsing numeric properties (temperature): " + ex.getMessage());
            throw new RuntimeException("Error parsing numeric properties", ex);
        } catch (NullPointerException ex) {
             System.err.println("Missing Env Variable OPENAI_API_KEY: " + ex.getMessage());
             throw new RuntimeException("Error loading Env variable OPENAI_API_KEY", ex);
        }
    }
    ...
    public void testOllamaChatModel() {
       ...
       */
       ...
       String message = """
        ...
        Message user = Message.builder().role(Message.Role.USER).content(message).build();
        List<Message> messages = Arrays.asList(system,user);
        ChatCompletion chatCompletion = ChatCompletion.builder().messages(messages).model(OPENAI_MODEL).temperature(OPENAI_TEMPERATURE).build();
